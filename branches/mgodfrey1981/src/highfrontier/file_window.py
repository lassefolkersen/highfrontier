...
class file_window():
                ...
                if self.position == "Load menu":
                    self.solar_system_object_link.load_solar_system(
                        os.path.join("savegames",
                                     self.distribute_click_to_subwindow.selected))
    ...
    def draw(self):
        ...
        for i, button_name in enumerate(self.button_list_now):
            if isinstance(self.button_structure[self.position][button_name], str): # has a submenu
                self.button_instances_now[button_name]=self.draw_submenu_button(
                    button_name,
                    self.button_structure[self.position][button_name],
                    (self.rect[2]-20,35),
                    (10+self.rect[0],i*40+10+self.rect[1]))
            else: # has a subfunction
                self.button_instances_now[button_name]=self.draw_subfunction_button(
                    button_name,
                    self.button_structure[self.position][button_name],
                    (self.rect[2]-20,35),
                    (10+self.rect[0],i*40+10+self.rect[1]))
    ...
    def go_to_submenu(self, function_parameter=[]):
        """
    ...
    def quit(self,function_parameter=[]):
        sys.exit(0)
...
#    def effectuate_load(self):
    ...
    def automation_settings(self,function_parameter=[]):
        """
...
#        self.manager.emit("update_infobox", None)
    ...
    def raise_waters(self,function_parameter=[]):
        """
    ...
    def lower_waters(self,function_parameter=[]):
        sol = self.solar_system_object_link
    ...
    def nuclear_war(self,function_parameter=[]):
        sol = self.solar_system_object_link
        if sol.display_mode == "planetary":
